...
class RAGSystemFactory:
    ...
    def create_default_system(self) -> RAGSystem:
        ...
        # Create vector store
        try:
            from ..components.vector_store.vector_store_factory import VectorStoreFactory
            
            # Use configuration to determine store type
            store_type = getattr(self.config, 'vector_store_type', 'local')
            
            if store_type == 'qdrant':
                # Create Qdrant store with fallback
                self.logger.info("Creating Qdrant vector store with local fallback...")
                components['vector_store'] = VectorStoreFactory.create_production_store()
                self.logger.info("Qdrant vector store created successfully (with fallback)")
            else:
                # Create local store
                components['vector_store'] = VectorStoreFactory.create_development_store()
                self.logger.info("Local vector store created successfully")
                
        except Exception as e:
            self.logger.warning(f"Failed to create vector store via factory: {e}")
            # Fallback to direct local store creation
            try:
                from ..components.vector_store.vector_store import VectorStore
                components['vector_store'] = VectorStore(storage_path=self.config.vector_store_path)
                self.logger.info("Fallback local vector store created successfully")
            except Exception as e2:
                self.logger.error(f"Failed to create fallback vector store: {e2}")
                # Create minimal fallback that prevents system failure
                from ..interfaces.base import VectorStoreInterface, FAQEntry, SimilarityMatch
                import numpy as np
                
                class _MinimalVectorStore(VectorStoreInterface):
                    """Minimal vector store that prevents system failure."""
                    def __init__(self):
                        self._stats = {'total_vectors': 0, 'store_type': 'minimal_fallback'}
                    
                    def store_vectors(self, vectors, document_id=None, document_hash=None):
                        self._stats['total_vectors'] = len(vectors)
                    
                    def search_similar(self, query_vector, threshold=0.7, top_k=10):
                        return []
                    
                    def search_by_ngrams(self, query_ngrams, threshold=0.9):
                        return []
                    
                    def search_with_filters(self, query_vector, threshold=0.7, top_k=10, **filters):
                        return []
                    ...
                    def health_check(self):
                        return {
                            'status': 'degraded',
                            'message': 'Using minimal fallback vector store',
                            'fallback_reason': 'vector_store_creation_failed'
                        }
                
                components['vector_store'] = _MinimalVectorStore()
                self.logger.warning("Created minimal fallback vector store to prevent system failure")
