...
class RAGSystem(RAGSystemInterface):
    ...
    def _retrieve_relevant_faqs(self, processed_query: ProcessedQuery, context) -> List[FAQEntry]:
        """Retrieve relevant FAQs with enhanced error handling, fallback mechanisms, and multiple search strategies."""
        retrieved_faqs = []
                ...
                if ngram_matches:
                    ...
                    for match in ngram_matches[:3]:
                        try:
                            ...
                            self.logger.warning(f"AI validation failed for FAQ '{match.faq_entry.question}': {e}")
                            # Include FAQ anyway if validation fails
                            validated_faqs.append(match.faq_entry)
        ...
        # Strategy 2: Vector Similarity Search with Enhanced Error Handling
        if self.vectorizer and self.vector_store:
            try:
                ...
                for attempt in range(self.config.embedding_retry_attempts):
                    try:
                        ...
                        if attempt < self.config.embedding_retry_attempts - 1:
                            import time
                            time.sleep(self.config.embedding_retry_delay * (attempt + 1))  # Exponential backoff
                        else:
                ...
                if query_embedding is not None:
                    ...
                    self.logger.warning("Could not generate query embedding, skipping vector search")
                
            except Exception as e:
                self.logger.warning(f"Vector similarity search failed: {e}")
                # Continue to fallback strategies
        
        # Strategy 3: Text-based Fallback Search (when embeddings fail)
        if not retrieved_faqs and self.config.text_search_fallback_enabled and self.vector_store:
            try:
                self.logger.info("Attempting text-based fallback search...")
                
                # Use N-gram search with lower threshold as fallback
                if processed_query.ngram_keywords:
                    fallback_matches = self.vector_store.search_by_ngrams(
                        processed_query.ngram_keywords, 
                        threshold=0.5  # Lower threshold for fallback
                    )
                    retrieved_faqs = [match.faq_entry for match in fallback_matches[:self.config.max_results]]
                    
                    if retrieved_faqs:
                        self.logger.info(f"Text fallback search found {len(retrieved_faqs)} FAQs")
                
            except Exception as e:
                self.logger.warning(f"Text-based fallback search failed: {e}")
        
        # Strategy 4: Use expanded queries if main query failed
        if not retrieved_faqs and processed_query.expanded_queries and self.vectorizer and self.vector_store:
            for expanded_query in processed_query.expanded_queries[:3]:  # Try top 3 expansions
                try:
                    ...
                    # Try to generate embedding for expanded query
                    expanded_embedding = None
                    try:
                        expanded_embedding = self.vectorizer.generate_embeddings(expanded_query)
                    except Exception as e:
                        self.logger.debug(f"Failed to generate embedding for expanded query: {e}")
                        continue
        ...
        # Strategy 5: Emergency fallback - return some FAQs to avoid "I don't know"
        if not retrieved_faqs and self.vector_store:
            try:
                self.logger.warning("No FAQs found through normal search, attempting emergency fallback...")
                
                # Get some recent or high-confidence FAQs as last resort
                emergency_stats = self.vector_store.get_vector_stats()
                if emergency_stats.get('total_vectors', 0) > 0:
                    # Try to get any FAQs with basic keyword matching
                    query_words = processed_query.corrected_query.lower().split()
                    if query_words:
                        # Use the first few words as emergency keywords
                        emergency_keywords = query_words[:3]
                        emergency_matches = self.vector_store.search_by_ngrams(
                            emergency_keywords,
                            threshold=0.1  # Very low threshold for emergency
                        )
                        
                        if emergency_matches:
                            retrieved_faqs = [match.faq_entry for match in emergency_matches[:3]]
                            self.logger.info(f"Emergency fallback found {len(retrieved_faqs)} FAQs")
        ...
        # Log final result
        if retrieved_faqs:
            self.logger.info(f"Successfully retrieved {len(retrieved_faqs)} relevant FAQs")
        else:
            self.logger.warning("No relevant FAQs found despite all fallback strategies")
    ...
    def _generate_fallback_response(self, query: str, error_msg: str, query_id: Optional[str] = None, processed_query: Optional[ProcessedQuery] = None) -> Response:
        """Generate an enhanced fallback response when embeddings or search fails."""
        
        # Try to provide a more helpful response based on the query content
        query_lower = query.lower()
        
        # Check for common question patterns and provide relevant guidance
        if any(word in query_lower for word in ['how', 'what', 'where', 'when', 'why', 'who']):
            fallback_text = (
                f"I understand you're asking about '{query}', but I'm having trouble finding the most relevant information right now. "
                "This might be due to a temporary system issue. Here are some suggestions:\n\n"
                "• Try rephrasing your question with different keywords\n"
                "• Check if your question relates to our main service areas\n"
            ...
            fallback_text = (
                f"I see you're looking for help with '{query}'. While I'm experiencing some technical difficulties, "
                "I want to make sure you get the assistance you need:\n\n"
                "• For urgent issues, please contact our support team directly\n"
                "• Try browsing our help documentation\n"
                "• You can also try asking your question in a different way\n\n"
                "Thank you for your understanding."
            )
        elif any(word in query_lower for word in ['price', 'cost', 'fee', 'payment', 'billing']):
            fallback_text = (
                f"I understand you have questions about pricing or billing related to '{query}'. "
                "While I'm currently unable to access the specific information you need, "
                "I recommend contacting our billing department directly for accurate and up-to-date information. "
                "They'll be able to provide you with detailed answers about costs, fees, and payment options."
            )
        else:
            fallback_text = (
                f"I apologize, but I'm currently experiencing technical difficulties while searching for information about '{query}'. "
                "This is likely a temporary issue with our search system. "
                "Please try rephrasing your question or contact our support team for immediate assistance. "
                "Thank you for your patience."
            )
        
        # Determine fallback reason based on error message
        fallback_reason = 'system_error'
        if 'embedding' in error_msg.lower():
            fallback_reason = 'embedding_failure'
        elif 'vector' in error_msg.lower() or 'qdrant' in error_msg.lower():
            fallback_reason = 'vector_store_failure'
        elif 'timeout' in error_msg.lower():
            fallback_reason = 'timeout_error'
        
        return Response(
            text=fallback_text,
            confidence=0.2,  # Slightly higher confidence for helpful fallback
            source_faqs=[],
            context_used=False,
            query_id=query_id,
            processed_query=processed_query,
            ...
            metadata={
                ...
                'original_query': query,
                'fallback_type': 'helpful_guidance'
            }
        )
