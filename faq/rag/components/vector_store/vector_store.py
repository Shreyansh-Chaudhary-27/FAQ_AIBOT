...
class VectorStore(VectorStoreInterface):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            health_status = {
                'status': 'healthy',
                'store_type': 'local_vector_store',
                'vector_count': len(self._vectors),
                'total_vectors': self._stats.get('total_vectors', 0),
                'last_updated': self._stats.get('last_updated'),
                'search_count': self._stats.get('search_count', 0),
                'average_search_time': self._stats.get('average_search_time', 0.0),
                'memory_usage_mb': self._estimate_memory_usage(),
                'storage_path': str(self.storage_path),
                'index_status': 'needs_rebuild' if self._needs_rebuild else 'current',
                'vector_dimensions': self._get_vector_dimensions(),
                'document_count': len(self._document_hashes),
                'qdrant_available': False,  # Local store doesn't use Qdrant
                'errors': []
            }
            
            # Check storage path accessibility
            if not self.storage_path.exists():
                health_status['errors'].append('Storage path does not exist')
                health_status['status'] = 'degraded'
            elif not os.access(self.storage_path, os.W_OK):
                health_status['errors'].append('Storage path is not writable')
                health_status['status'] = 'degraded'
            
            # Check if vectors are consistent with metadata
            vector_ids = set(self._vectors.keys())
            metadata_ids = set(self._metadata.keys())
            if vector_ids != metadata_ids:
                health_status['errors'].append('Vector and metadata IDs are inconsistent')
                health_status['status'] = 'degraded'
            
            # Check for very old data (potential staleness)
            if self._stats.get('last_updated'):
                from datetime import datetime, timedelta
                if datetime.now() - self._stats['last_updated'] > timedelta(days=7):
                    health_status['errors'].append('Data may be stale (not updated in 7+ days)')
                    health_status['status'] = 'degraded'
